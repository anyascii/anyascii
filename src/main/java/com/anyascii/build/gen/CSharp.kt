...
fun cSharp(g: Generator) {
    Path.of("impl/csharp/src/Transliteration.data.cs").bufferedWriter().use { w ->
        ...
        w.write("};\n")
        w.write("\t\tprivate static System.ReadOnlySpan<byte> Bank2 => new byte[]{")
        w.write(g.bank2.asSequence().joinToString(",") { it.code.toString() })
        w.write("};\n")
        w.write("\t\tprivate const int Bank2Length = $BANK2_LENGTH;\n\n")
